...
Quiz3Correct::Quiz3Correct(SceneManager& manager) :
	Scene(manager),
	updateFunc_(&Quiz3Correct::FadeInUpdate),
	letterH_(-1),
	interval_(0),
	interval2_(0)
{
...
void Quiz3Correct::FadeInUpdate(const InputState& input)
{
	fadeValue_ = 255 * static_cast<float>(fadeTimer_) / static_cast<float>(kFadeInterval);
	if (--fadeTimer_ == 0)
	{
		updateFunc_ = &Quiz3Correct::NormalUpdate;
	}
}

void Quiz3Correct::NormalUpdate(const InputState& input)
{
	//選択肢を上下で回す処理
	...
	if (currentInputIndex_ == 0)
	{
		...
		if (input.IsTriggered(InputType::next))
		{
			//正解
			whichAnswer_ = true;
			updateFunc_ = &Quiz3Correct::FadeOutUpdate;
		}
	}
	//選択肢1が選択中の場合
...
void Quiz3Correct::FadeOutUpdate(const InputState& input)
{
	fadeValue_ = 255 * (static_cast<float>(fadeTimer_) / static_cast<float>(kFadeInterval));
...
void Quiz3Correct::Update(const InputState& input)
{
	(this->*updateFunc_)(input);
}

void Quiz3Correct::Draw()
{
	...
	DrawStringToHandle(pw_start_x + 140, pw_start_y + 100, L"正解", GetColor(255, 0, 0), correctH_);
